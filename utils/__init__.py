...
def import_state_store_class(db_module_class: str):
    """
    Imports and returns the `DB` class from a specified database module.

    Args:
        db_module_class (str): The module path for the database class to import.

    Returns:
        module.DB: The `DB` class from the imported module.

    Raises:
        Exception: If the module cannot be imported or does not contain a `DB` class.
    """
    try:
        ...
        log_exception(
            f"Error importing {db_module_class}",
            sys.exc_info()
        )
        sys.exit(1)

def get_system_config(
    sys_config_file: str = None,
    section: str = None
) -> dict:
    """
    Reads and parses a system configuration file into a dictionary.

    Args:
        sys_config_file (str, optional): The path to the system configuration file.
        section (str, optional): The specific section of the configuration to retrieve.

    Returns:
        dict: A dictionary containing the parsed configuration data. If `section` is provided,
              returns the dictionary for that specific section only.
    ...
    def parse_list(data: str) -> list:
        """
        Takes a string and splits it into a list of items, stripping whitespace from
        each item and ignoring empty lines.

        Args:
            data (str): The string to parse.

        Returns:
            list: A list of strings, stripped of whitespace and with empty lines removed.
        """
        return [
            item.strip()
    ...
    try:
        ...
        config_parser.read_file(sys_config_file, "r")

        sys_config = dict()
        for s in config_parser.sections():
            sys_config[s] = dict(config_parser.items(s))
        ...
        for k, v in sys_config["status"].items():
...
def log_ini(
    ...
    """
    Initialises the logging module for the given script.

    Args:
        script (str): The name of the script to log as.
        level (int, optional): The logging level to use. Defaults to logging.INFO.
        to_disk (bool, optional): Whether to log to disk as well as the console. Defaults to True.
    ...
    if to_disk:
        handlers.append(
            TimeRotatingFileHandler(
                os.path.join(FOLDER_LOGS, f"{script}{EXTENSION_LOG}"),
                when="midnight",
                backupCount=2,
                encoding="utf8",
            )
...
def log_event_received(event) -> None:
    """
    Logs a received event to the console.

    Args:
        event (confluent_kafka.Message): The event to log.
    """
    event_data = [
        event.topic(),
        event.key(),
    ...
    for n in range(len(event_data)):
        try:
            event_data[n] = event_data[n].decode("utf-8")
        except Exception:
            pass
    logging.info(  
        f"Event received: {event_data}"
        )
...
def log_exception(message: str, sys_exc_info) -> None:
    ...
    Args:
        message (str): An optional message to log before the exception.
        sys_exc_info (tuple): A tuple containing the exception information, as returned by sys.exc_info().
    """
    exc_type, exc_tb, exc_tb = sys_exc_info
    fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
    logging.critical(f"{message} {exc_type} {fname} {exc_tb.tb_lineno}")



def validate_cli_args(script: str) -> tuple:
    """
    Validates the command line arguments for the given script.

    Args:
        script (str): The name of the script to validate arguments for.

    Returns:
        tuple: A tuple containing the paths to the Kafka configuration file and the system configuration file.

    Raises:
        SystemExit: If the configuration files are not found.
    """
...
def get_script_name(file: str) -> str:
    """Gets the name of the given script file, without any path or extension."""
...
def get_string_status(status_dict: dict, status: int) -> str:
    """
    Gets a string representation for a given status code, using the provided
    dictionary mapping. If the status is not found, it will return the value
    associated with the key `None` in the dictionary, or else a default string
    including the status code.

    Args:
        status_dict (dict): Mapping of status codes to their string
            representations.
        status (int): The status code to get the string for.

    Returns:
        str: The string representation of the status code.
    """
...
def set_producer_consumer(
    ...
    def main():
        kafka_config_file = "kafka.ini"
        producer_extra_config = {"bootstrap.servers": "localhost:9092"}
        consumer_extra_config = {"group.id": "my-consumer-group"}
        disable_producer = False
        ...
        print(f"Admin client: {admin_client}")
    
    if __name__ == "__main__":
        main()
        => All config: {'kafka': {'bootstrap.servers': 'localhost:9092', 'security.protocol': 'PLAINTEXT'}}
        Producer: <kafka.producer.Producer object at 0x10a798d10>
        Consumer: <kafka.consumer.Consumer object at 0x10a798f90>
        Admin client: <kafka.admin.AdminClient object at 0x10a798d90>"""
...
def ksqldb(
    ...
    ksqlDB(
        end_point="http://localhost:8088",
        statement="CREATE STREAM my_stream (id INT, name VARCHAR) WITH (kafka_topic='my_topic', value_format='json');",
        offset_reset_earliest=True
    )
    DEBUG: ksqlDB (200): CREATE STREAM my_stream (id INT, name VARCHAR) WITH (kafka_topic='my_topic', value_format='json');
    """
    try:
        # Clean-up statement
